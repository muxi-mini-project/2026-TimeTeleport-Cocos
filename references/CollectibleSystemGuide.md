...
在 `LevelMapManager` 组件的 Inspector 中，可以为每种收集物类型配置独立的预制体：

| 属性 | 说明 |
|------|------|
| `collectiblePrefab` | 通用后备预制体（任何类型未配置时使用） |
| `fragmentPrefab` | 时间碎片专用预制体 |
| `chipPrefab` | 未来芯片专用预制体 |
| `fossilPrefab` | 远古化石专用预制体 |
| `letterPrefab` | 信件专用预制体 |

### 预制体选择优先级

```
1. 类型专用预制体（如 chipPrefab）
     ↓ 如果未配置
2. 通用后备预制体（collectiblePrefab）
     ↓ 如果未配置
3. 跳过该收集物并输出警告
```
